...
String Timezone::getPosix() { return _posix; }
	...
	String Timezone::getOlsen() {
		...
		bool Timezone::readCache(String &olson, String &posix, uint8_t &months_since_jan_2018) {
				...
				uint8_t nvs_first_space = read_string.indexOf(' ');
				uint8_t nvs_second_space = read_string.indexOf(' ', nvs_first_space + 1);
				if (nvs_first_space && nvs_second_space) {
					months_since_jan_2018 = read_string.toInt();
					posix = read_string.substring(nvs_second_space + 1);
					olson = read_string.substring(nvs_first_space + 1, nvs_second_space);
					info(F("Cache read. Olson: ")); info(olson); info (F("  Posix: ")); infoln(posix);
					return true;
				}
				return false;
